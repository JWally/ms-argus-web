 ...
 *
 * Test values and expected results for floating-point precision detection.
 *
...
import type { MathTestCase } from './types';
...
/**
 * Math function test cases with expected browser-specific results.
 *
 * Format: [functionName, args, displayName, chrome, firefox, torBrowser, safari]
 *
 * Why this works:
 * - Floating-point operations have rounding decisions at the edge of precision
 * - Different engines make different rounding choices
 * - V8 (Chrome), SpiderMonkey (Firefox), and JavaScriptCore (Safari) differ
 * - Tor Browser uses Firefox with privacy modifications that affect some results
 *
 * NaN in expected values means "same as Chrome" (the reference implementation).
 */
export const MATH_TEST_CASES: MathTestCase[] = [
  // acos tests
  ...
  // acosh tests - hyperbolic arccosine edge cases
  ...
  // asinh tests - hyperbolic arcsine with large values
  ...
  // cos tests - these show significant engine differences
  ...
  // expm1 tests - exp(x) - 1 with better precision for small x
  ...
  // hypot tests - Euclidean distance
  [
    'hypot',
    [1, 2, 3, 4, 5, 6],
    'hypot(1, 2, 3, 4, 5, 6)',
  ...
  // sin tests - these show Tor Browser differences
  [
    'sin',
    [bigN],
    `sin(${bigN})`,
  ...
  // pow tests - exponentiation with edge cases
  [
    'pow',
    [n, -100],
    `pow(${n}, -100)`,
