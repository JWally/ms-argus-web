...
function getProbeUrl(index: number, baseUrl: string, probePath: string): string {
 ...
 */
async function readFromCacheAPI(numBits: number = FAVICON_BITS): Promise<string | null> {
  try {
 ...
 */
async function readWithTiming(config: FaviconCacheConfig): Promise<string | null> {
  const {
    bits = FAVICON_BITS,
    baseUrl = '',
  ...
  try {
    const results = await probeAllWithTiming(baseUrl, probePath, bits, timingThreshold)
...
export async function getFaviconCacheId(
  ...
  inflightPromise = (async () => {
    ...
    if (meta && Date.now() - new Date(meta.lastSeen).getTime() < 7 * 24 * 60 * 60 * 1000) {
 ...
 */
export async function setFaviconCacheId(hexId: string): Promise<FaviconCacheData> {
