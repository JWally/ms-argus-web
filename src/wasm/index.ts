...
export default async function getWasmFingerprint(
  ...
  try {
    ...
    const workerRatio = opts.fast ? null : await probes.workerRatio(cpu, workerIterations)

    logTestResult({ time: timer.stop(), test: 'wasm fingerprint', passed: true })
    ...
    logTestResult({ time: timer.stop(), test: 'wasm fingerprint', passed: false })
...
export { loadWasm, detectSimd, isLoaded, getWasm, getWasmBytes } from './loader'
